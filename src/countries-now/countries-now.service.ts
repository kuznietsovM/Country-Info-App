...
export class CountriesNowService {
    ...
    constructor(
        ...
        const host = configService.get<string>('COUNTRIES_NOW_HOST', 'countriesnow.space')
        const version = configService.get<string>('COUNTRIES_NOW_VERSION', 'v0.1')
        this.origin = `https://${host}/api/${version}`
    }

    async getCountryPopulationData (countryName: string) : Promise<CountryPopulation> {
        try {
            const {data: response} = await firstValueFrom(
                this.httpService.post<CountriesNowResponse<CountryPopulation>>(`${this.origin}/countries/population`, {
                    country: countryName
                })
            )
    ...
    async getCountryFlagUrl (countryCode: string) {
        try {
            const {data: response} = await firstValueFrom(
                this.httpService.post<CountriesNowResponse<CountryFlag>>(`${this.origin}/countries/flag/images`, {
                    iso2: countryCode
                })
            )
